...
class RecordingService:
    ...
    def get_talents(self) -> list[RecordingTalentOption]:
        with self.database.connect() as connection:
            rows = connection.execute(
                ...
                FROM talents AS t
                JOIN dialog_cast AS dc ON dc.talent_id = t.id
                JOIN dialogues AS d ON d.id = dc.dialogue_id
                JOIN episodes AS e ON e.id = d.episode_id
                WHERE t.is_active = 1 AND d.is_active = 1 AND e.is_active = 1
                ORDER BY t.name COLLATE NOCASE, t.id
                """
            ).fetchall()
        return [RecordingTalentOption(id=int(r["id"]), name=str(r["name"])) for r in rows]
    ...
    def get_characters(self) -> list[RecordingCharacterOption]:
        with self.database.connect() as connection:
            rows = connection.execute(
                ...
                JOIN characters AS c ON c.id = COALESCE(ca.canonical_character_id, dc.character_id)
                JOIN dialogues AS d ON d.id = dc.dialogue_id
                JOIN episodes AS e ON e.id = d.episode_id
                WHERE c.is_active = 1 AND d.is_active = 1 AND e.is_active = 1
                ORDER BY c.name COLLATE NOCASE, c.id
                """
            ).fetchall()
        return [RecordingCharacterOption(id=int(r["id"]), name=str(r["name"])) for r in rows]

    def get_characters_for_talent(self, talent_id: int) -> list[RecordingCharacterOption]:
        with self.database.connect() as connection:
            rows = connection.execute(
                """
                SELECT DISTINCT c.id, c.name
    ...
    def get_episodes_for_character(self, character_id: int) -> list[RecordingEpisodeOption]:
        with self.database.connect() as connection:
            rows = connection.execute(
                """
                SELECT DISTINCT e.episode_number
                FROM episodes AS e
                JOIN dialogues AS d ON d.episode_id = e.id
        ...
        return [RecordingEpisodeOption(episode_number=int(r["episode_number"])) for r in rows]

    def get_episodes_for_cast(self, *, talent_id: int, character_id: int) -> list[RecordingEpisodeOption]:
        with self.database.connect() as connection:
            rows = connection.execute(
                """
                SELECT DISTINCT e.episode_number
                ...
                ORDER BY e.episode_number
                """,
                (int(character_id), int(talent_id)),
            ).fetchall()
        return [RecordingEpisodeOption(episode_number=int(r["episode_number"])) for r in rows]

    def get_episode_cast(self, episode_number: int) -> list[EpisodeCastItem]:
        with self.database.connect() as connection:
            rows = connection.execute(
                """
                SELECT DISTINCT
        ...
        return [
            EpisodeCastItem(
                ...
                talent_name=str(r["talent_name"] or ""),
            )
            for r in rows
        ]
    ...
    def get_dialogues(
        ...
        *,
        ...
        cast_conditions = ["COALESCE(ca.canonical_character_id, dc.character_id) = ?"]
        params: list[object] = [int(episode_number), int(character_id)]
        ...
        with self.database.connect() as connection:
            rows = connection.execute(
                ...
                ORDER BY d.time_in, d.source_row, d.id
                """,
                params,
            ).fetchall()
        return [
            RecordingDialogueRow(
                ...
                is_recorded=bool(r["is_recorded"]),
            )
            for r in rows
        ]

    def get_source_file_path(self, episode_number: int) -> str:
        with self.database.connect() as connection:
            row = connection.execute(
                ...
                WHERE e.episode_number = ? AND e.is_active = 1 AND sf.is_active = 1
                LIMIT 1
                """,
                (int(episode_number),),
            ).fetchone()
        return str(row["file_path"] or "") if row else ""

    def set_recorded(self, dialogue_id: int, recorded: bool) -> None:
        timestamp = datetime.now().isoformat(timespec="seconds")
        with self.database.connect() as connection:
            exists = connection.execute(
                "SELECT 1 FROM dialogues WHERE id = ? AND is_active = 1",
                (int(dialogue_id),),
            ).fetchone()
            if exists is None:
                raise ValueError(f"Dialogue {dialogue_id} tidak ditemukan atau sudah inactive.")
            connection.execute(
                """
                ...
                """,
                (int(dialogue_id), 1 if recorded else 0, timestamp if recorded else None, timestamp),
            )

    def set_recorded_bulk(self, dialogue_ids: list[int] | tuple[int, ...], recorded: bool) -> int:
        ...
        with self.database.connect() as connection:
            active_rows = connection.execute(
                f"SELECT id FROM dialogues WHERE is_active = 1 AND id IN ({placeholders})",
                ids,
            ).fetchall()
            ...
            connection.executemany(
                ...
                """,
                [
                    (dialogue_id, 1 if recorded else 0, timestamp if recorded else None, timestamp)
