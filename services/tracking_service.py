...
class TrackingService:
    ...
    def get_talents(self) -> list[TrackingOption]:
        with self.database.connect() as connection:
            rows = connection.execute(
                ...
                JOIN dialogues AS d ON d.id = dc.dialogue_id AND d.is_active = 1
                JOIN episodes AS e ON e.id = d.episode_id AND e.is_active = 1
                WHERE t.is_active = 1
                ORDER BY t.name COLLATE NOCASE, t.id
                """
            ).fetchall()
        return [TrackingOption(id=int(r["id"]), label=str(r["name"])) for r in rows]
        ...
        *,
        ...
        query = f"""
            SELECT
                ...
                COUNT(DISTINCT CASE WHEN COALESCE(rs.is_recorded, 0) = 1 THEN d.id END)
            ...
            JOIN characters AS c
              ON c.id = COALESCE(ca.canonical_character_id, dc.character_id)
             AND c.is_active = 1
            JOIN talents AS t ON t.id = dc.talent_id AND t.is_active = 1
            ...
            GROUP BY
                ...
                t.id, t.name,
                ss.status
            ORDER BY c.name COLLATE NOCASE, c.id, e.episode_number
        """
    ...
    def get_characters_to_stem(self, talent_id: int, episode_number: int) -> list[TrackingChip]:
        rows = self.get_character_rows(talent_id, episode_number=episode_number)
    ...
    def set_downstream_status(
        ...
        *,
        ...
        with self.database.connect() as connection:
            ...
            if normalized_status in {READY_TO_STEM, STEMMED, DELIVERED} and recorded < total:
                raise ValueError(
            ...
            if normalized_status == NOT_READY:
                connection.execute(
                    ...
                    """,
                    (int(episode_id), int(talent_id), int(character_id)),
                )
                return
    ...
    def _get_progress(
        ...
        *,
        ...
        if row is None:
            return 0, 0
        return int(row["total_dialogues"] or 0), int(row["recorded_dialogues"] or 0)
