...
def test_tracking_keeps_compact_matrix_as_primary_workspace() -> None:
    ...
    assert "layout.addWidget(self.scroll, 1)" in compact
...
def test_tracking_filters_and_legend_are_compact_and_visible() -> None:
    ...
    assert "READY_TO_STEM," in base.split("STATUS_ORDER = (", 1)[1].split(")", 1)[0]
...
def test_tracking_footer_stays_secondary_to_episode_matrix() -> None:
    ...
    assert 'QLabel("CHARACTER TO STEM")' in compact
    assert "self.character_table.setMaximumHeight(112)" in compact
    assert 'QPushButton("Go to Output Health")' in compact
    assert "footer.addWidget(queue_panel, 1)" in compact
    assert "footer.addWidget(self.output_health_summary, 0)" in compact
