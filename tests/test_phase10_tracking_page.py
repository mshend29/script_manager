...
def test_tracking_keeps_compact_matrix_as_primary_workspace() -> None:
    ...
    assert '"TrackingWorkspaceTabs"' not in compact.split("def _build_tracking_workspaces", 1)[1].split("def _build_tracking_grid_workspace", 1)[0]
...
def test_tracking_footer_stays_secondary_to_episode_matrix() -> None:
    ...
    assert 'QLabel("TRACK UNTUK STEM")' in compact
    assert "self.character_table.setMaximumHeight(112)" not in compact
    assert "body.addWidget(queue_panel)" in compact
    assert "queue_layout.addLayout(episode_row)" in compact
    assert "get_episodes_for_talent(int(talent_id))" in _read("pages/tracking_page.py")
    assert "queue_layout.addWidget(self.output_health_summary, 0)" not in compact
