...
def test_validation_is_detailed_filterable_and_actionable():
    ...
    for header in ("TINGKAT", "KATEGORI", "EPS", "ENTITAS", "KODE", "PESAN", "AKSI"):
        assert header in source
    assert "Semua Tingkat" in source
    assert "Semua Kategori" in source
    assert "Semua Episode" in source
    assert "Cari validasi…" in source
...
def test_validation_tracking_action_is_wired_to_main_window():
    ...
    assert 'self.set_page("TRACKING")' in window
...
def test_overview_distinguishes_review_system_and_workflow_health():
    ...
    assert "Narasi / Non-Dialog" in source
    assert "Perlu Ditinjau" in source
    assert "Error Sistem" in source
    assert "Peringatan Alur" in source
    assert "✓ Data proyek sehat." in source
